...
def fit_earth(max_iter=20000,
    ...
    print(pos_idx.shape, pos.shape, uv_idx.shape, uv.shape, tex.shape)
    ...
    for it in range(max_iter + 1):
        ...
        r_rot = nvutil.random_rotation_translation(0.25)

        # Smooth rotation for display.
        a_rot = np.matmul(nvutil.rotate_x(-0.4), nvutil.rotate_y(ang))
        dist = np.random.uniform(0.0, 48.5)

        # Modelview and modelview + projection matrices.
        proj = nvutil.projection(x=0.4, n=1.0, f=200.0)
        r_mv = np.matmul(nvutil.translate(0, 0, -1.5 - dist), r_rot)
        r_mvp = np.matmul(proj, r_mv).astype(np.float32)
        a_mv = np.matmul(nvutil.translate(0, 0, -3.5), a_rot)
        ...
        while color.shape[1] > res:
            color = nvutil.bilinear_downsample(color)
        ...
        if display_image or save_image:
            ...
            with torch.no_grad():
                ...
                if display_image:
                    nvutil.display_image(result_image, size=display_res, title='%d / %d' % (it, max_iter))
                if save_image:
                    nvutil.save_image(out_dir + '/' + (imgsave_fn % it), result_image)

                if save_texture:
                    texture = tex_opt.cpu().numpy()[::-1]
                    nvutil.save_image(out_dir + '/' + (texsave_fn % it), texture)
...
def main():
    ...
    parser.add_argument('--outdir', help='Specify output directory', default='run')
    ...
    # Run.
    fit_earth(max_iter=args.max_iter, log_interval=10, display_interval=args.display_interval, enable_mip=args.mip, out_dir=out_dir, log_fn='log.txt', texsave_interval=1000, texsave_fn='tex_%06d.png', imgsave_interval=1000,
