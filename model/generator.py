...
class Generator(torch.nn.Module):
    ...
    def __init__(self, z_dim, w_dim, w_num_layers, img_resolution, img_channels, mipmap=True, channel_base=16384, channel_max=512):
        super().__init__()
        self.z_dim = z_dim
        self.w_dim = w_dim
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, mipmap=mipmap, channel_base=channel_base, channel_max=channel_max)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(self, w_dim, img_resolution, img_channels, mipmap, channel_base, channel_max):
        super().__init__()
        ...
        for res in self.block_resolutions[1:]:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res, img_channels=img_channels, mipmap=mipmap)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, resolution, img_channels, mipmap):
        super().__init__()
...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, w_dim, resolution, kernel_size=1, resampler=identity, activation='lrelu'):
        super().__init__()
        self.resolution = resolution
