...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.G = Generator(config.latent_dim, config.latent_dim, config.num_mapping_layers, config.texture_size, 3)
    ...
    def render(self, texture_image):
        texture_image = texture_image.permute((0, 2, 3, 1)).mean(0).contiguous()
        vertices, uvs = [], []
        faces, uv_indices = [], []
        ...
        for i in range(self.config.batch_size):
            ...
            t_v = transform_pos_mvp(self.train_set.vertices, torch.matmul(projection_matrix, view_matrix).to(self.device).unsqueeze(0))
            vertices.append(t_v)
    ...
    def export_images(self, prefix, output_dir_grid, output_dir_samples, output_dir_texture, output_dir_fid):
        ...
        for iter_idx in range(self.config.num_eval_images // self.config.batch_size):
            latent = self.latent()
            texture_atlas = self.G(latent, noise_mode='const')
