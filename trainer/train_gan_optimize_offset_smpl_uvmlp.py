...
from model.uvmlp import SimpleImplicitDecoder
...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.R = None
        self.train_set = RenderSMPLOffsetDataset("data/smpl_0/smpl.obj", "data/smpl_0/offset.obj", "data/smpl_0/laplacian.npz", "data/smpl_0/inverse_laplacian.npz", "data/smpl_0/extended.pkl", 2048)
        self.val_set = RenderSMPLOffsetDataset("data/smpl_0/smpl.obj", "data/smpl_0/offset.obj", "data/smpl_0/laplacian.npz", "data/smpl_0/inverse_laplacian.npz", "data/smpl_0/extended.pkl", config.num_eval_images)
        self.G = SimpleImplicitDecoder(3, config.texture_size)
        self.automatic_optimization = False
        self.ema = None

    def configure_optimizers(self):
        g_opt = torch.optim.Adam(self.G.parameters(), lr=self.config.lr_g, betas=(0.0, 0.99), eps=1e-8)
    ...
    def g_regularizer(self):
        g_opt = self.optimizers()[0]
        g_opt.zero_grad(set_to_none=True)
        offsets = self.sample_offset_map(self.scale_output(self.G()))
    ...
    def sample_offset_map(self, offset_map):
        offsets = torch.nn.functional.grid_sample(offset_map.permute((2, 0, 1)).unsqueeze(0), self.train_set.uv.reshape((1, -1, 1, 2)), mode='bicubic', align_corners=True)
        return offsets.squeeze(0).squeeze(-1).T.contiguous()

    def render(self, offset_map):
        # sample the offsets at the uv coordinates
        sampled_offsets = self.sample_offset_map(offset_map)
        vertices, faces, normals = [], [], []
        normals_ = vertex_to_normals(self.train_set.vertices_smpl + sampled_offsets, self.train_set.faces.long())
        num_vertex = 0
        start_index = 0
        ranges = []
        for i in range(self.config.batch_size):
            projection_matrix, view_matrix = get_random_view()
            t_v = transform_pos_mvp(self.train_set.vertices_smpl + sampled_offsets, torch.matmul(projection_matrix, view_matrix).to(self.device).unsqueeze(0))
            vertices.append(t_v)
    ...
    def export_images(self, prefix, output_dir_grid, output_dir_samples, output_dir_texture, output_dir_fid):
        ...
        for iter_idx in range(self.config.num_eval_images // self.config.batch_size):
            offset_map = self.scale_output(self.G())
            if iter_idx == 0:
                self.export_mesh(self.sample_offset_map(offset_map), output_dir_grid)
            fake = self.render(offset_map).cpu()
            ...
            if iter_idx < self.config.num_vis_images // self.config.batch_size:
                ...
                if output_dir_texture is not None:
                    save_image(offset_map.permute((2, 0, 1)), output_dir_texture / f"{iter_idx}.jpg", value_range=(-1, 1), normalize=True)
