...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.R = None
        self.train_set = RenderSMPLOffsetDataset("data/smpl_0/smpl.obj", "data/smpl_0/offset.obj", "data/smpl_0/laplacian.npz", 2048)
        self.val_set = RenderSMPLOffsetDataset("data/smpl_0/smpl.obj", "data/smpl_0/offset.obj", "data/smpl_0/laplacian.npz", config.num_eval_images)
    ...
    def g_regularizer(self, warm_up_mode=False):
        g_opt = self.optimizers()[0]
        g_opt.zero_grad(set_to_none=True)
        z = self.latent()
        w = self.get_mapped_latent(z)
        fake = self.scale_output(self.G.synthesis(w))
        offsets = fake.permute((0, 2, 3, 1)).mean(0).reshape(-1, 3)[:self.train_set.vertices_smpl.shape[0], :].contiguous()
    ...
    def training_step(self, batch, batch_idx):
        if self.global_step > self.config.warm_up_steps:
            self.g_step()
            self.g_regularizer()
            self.d_step(batch)
            ...
            self.g_regularizer(warm_up_mode=True)
        self.ema.update(self.G.parameters())

    def render(self, offset_map):
        offset_map = offset_map.permute((0, 2, 3, 1)).mean(0).reshape(-1, 3)[:self.train_set.vertices_smpl.shape[0], :].contiguous()
    ...
    def export_mesh(self, offset_map, output_dir_grid):
        offset_map = offset_map.permute((0, 2, 3, 1)).mean(0).reshape(-1, 3)[:self.train_set.vertices_smpl.shape[0], :].contiguous()
        vertices = self.train_set.vertices_smpl + offset_map
